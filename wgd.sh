...
help() {
  ...
  display_logo2
  printf "${YELLOW}=================================================================================\n"
  printf "${YELLOW}+     ${BLUE}<Wireguard Master> by Iliya ${BLUE}https://github/Iliya8989/wg-master        ${YELLOW}+\n"
  printf "${YELLOW}=================================================================================${NC}\n"
...
update_wgd() {
  new_ver=$(python3 -c "import json; import urllib.request; data = urllib.request.urlopen('https://api.github.com/repos/Iliya8989/wg-master.git').read(); output = json.loads(data);print(output['tag_name'])")
  printf "%s\n" "$dashes"
  ...
  if [ "$up" = "Y" ]; then
    ...
    # git pull https://github.com/Iliya8989/wg-master.git $new_ver --force >  /dev/null 2>&1
